...
def define_UNet_Architecture(imageSize, numClasses, filterMultiplier=10):
    ...
    for layer_index in range(num_layers):
        ...
        down_filter_numbers[layer_index] = int(
            (layer_index + 1) * filterMultiplier + numClasses
        )
        up_filter_numbers[layer_index] = int(
            (num_layers - layer_index - 1) * filterMultiplier + numClasses
        )
...
#############################################################################################################
#    Generates your simulated images and segmentations.
#############################################################################################################
    ...
    for vol_file in os.listdir(dataSetPath):
        if vol_file.endswith(".npy") and not vol_file.endswith("_segmentation.npy"):
            ...
            seg_dir = os.path.join(
                dataSetPath, seg_file.replace("_segmentation.npy", "_segmentation")
            )
            os.makedirs(vol_dir, exist_ok=True)
            os.makedirs(seg_dir, exist_ok=True)

            for i in range(vol.shape[0]):
                vol_slice_path = os.path.join(
                    vol_dir, vol_file.replace(".npy", "_") + f"{i:03}"
                )
                seg_slice_path = os.path.join(
                    seg_dir,
                    seg_file.replace("_segmentation.npy", "_segmentation_") + f"{i:03}",
                )
                vol_slice = vol[i, :, :]
                seg_slice = seg[i, :, :]
...
#############################################################################################################
#  Returns your data divided into 3 non-overlapping sets
#############################################################################################################
def splitDataIntoSets(images, segmentations):
    ...
    trainData = [
        (image, seg)
        for image, seg, pid in zip(images, segmentations, patientIDs)
        if pid in trainIDs
    ]
    valData = [
        (image, seg)
...
#############################################################################################################
#    Creates a plot for the training and validation loss/metrics.
#    Training and validation shown on the same graph
#############################################################################################################
...
def main():
    ...
    earlyStoppingCallback = tensorflow.keras.callbacks.EarlyStopping(
        monitor="val_accuracy", mode="max", verbose=1, patience=5
    )
    modelCheckPointCallback = tensorflow.keras.callbacks.ModelCheckpoint(
        modelName,
        verbose=1,
    ...
    unet.compile(
        optimizer=optimizer, loss=lossFunction, metrics=metrics, run_eagerly=True
    )
    ...
    #############################################################################################################
    # Generating predictions and evaluations here
    #############################################################################################################

    modelPath = (
        "/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/Run3/"
        + modelName
    )
    ...
    for pred in y_pred:
        ...
        numpy.save(
            "/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/unet_predictions2/Test_Images_Ultrasound_{:05d}".format(
                count
            ),
            pred,
        )
...
#############################################################################################################
#    Computes the mean hausdorff distance
#############################################################################################################
