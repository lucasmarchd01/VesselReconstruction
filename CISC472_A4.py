    ...
    for seed, seed_value in [(object_seed, 1), (background_seed, 0)]:
        ...
        while still_to_grow:
            ...
            for neighbor in neighbors:
                if (neighbor[1] >= border_size and neighbor[1] < image.shape[1]-border_size and 
                    neighbor[0] >= 0 and neighbor[0] < image.shape[0]):
                    if neighbor not in already_segmented:
                        if segmented_img[current] == 1:
                            if abs(int(image[neighbor]) - object_mean) <= maximum_difference:
                                segmented_img[neighbor] = seed_value
                                still_to_grow.append(neighbor)
                                already_segmented.add(neighbor)
                        else:
                            if abs(int(image[neighbor]) - background_mean) <= maximum_difference:
                                segmented_img[neighbor] = seed_value
                                still_to_grow.append(neighbor)
                                already_segmented.add(neighbor)
...
def find_probeToReference(filename):
    ...
    """
    probeToRefTimes = pd.read_csv("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/transforms/ProbeToReferenceTimeStamps.csv")
    testImgTimes = pd.read_csv("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/Test_Images/Case_05/Test_Images_Ultrasound_Labels.csv")
    ...
    for i in probeToRefTimes.index:
        if probeToRefTimes["Time"][i] == time:
            probeToReference = np.load(os.path.join("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/transforms/ProbeToReference/", probeToRefTimes["Filepath"][i]))
...
def transform_contour_points(contours, img_filename):
    ...
    imageToProbe = np.load("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/transforms/ImageToProbe.npy")
    referenceToRAS  = np.load("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/transforms/ReferenceToRAS.npy")
    size = contours.shape[0]
    contours_point = np.hstack((contours, np.zeros((size, 1)), np.ones((size, 1))))
    point_ras = np.matmul(np.matmul(np.matmul(contours_point, imageToProbe), probeToRef), referenceToRAS)
    ras = point_ras[:, :3]
    return ras
...
if __name__ == "__main__":
    ...
    train_images = getImages("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/Training_Images")
    ...
    for i in range(len(test_images)):   
        ...
        np.save("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/pointbased_predictions/Test_Images_Ultrasound_{:05d}".format(count), segmentation)
        count += 1
    ...
    for i in range(len(test_images)):
        ...
        display_image(test_segmentations[i+60])
        display_image(segmentation)
        np.save("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/regionbased_predictions/Test_Images_Ultrasound_{:05d}".format(count), segmentation)
        count += 1
    
    # Ground truth segmentations
    count = 1
    for i in range(len(test_segmentations)):
        np.save("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/groundtruth_segmentations/Test_Images_Ultrasound_{:05d}".format(count), test_segmentations[i])
        count += 1
    ...
    np.save("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/groundTruthContours.npy", groundTruthContours)
    ...
    np.save("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/thresholdingContours.npy", thresholding)


    dataset = "/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/unet_predictions2"
    unet = generate_contours(dataset)
    np.save("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/unetContours.npy", unet)
    ...
    np.save("/Users/lucasmarch/OneDrive/CISC 472/Assignment_4/Assignment4Data/regionGrowingContours.npy", region)
